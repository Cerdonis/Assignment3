    ...
    for j=win+1:c-win
        ...
        patch1 = PATCH(win*2+1, i, j, im1_crop);
         ...
         for k=j:min(j+lim, c-win)
             ...
             % NCC somehow took too long time to calculate.
%              ncc = normxcorr2(patch1, patch2);
%              temp_max = max(max(ncc));
%              
%              if ncc_max < temp_max
%                  ncc_max = temp_max;
%                  disparity_ncc(i,j) = k-j;
%              end
...
disparity_ncc=disparity_ncc/max(max(disparity_ncc));
